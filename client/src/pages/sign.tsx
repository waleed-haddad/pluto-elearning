...
export default function Sign() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('http://localhost:3010/api/signup/', formData);
      ...
      <div className="auth-real">
        ...
        <div className="auth-inputs">
          <div className="auth-input">
            <input className="auth-input-box" type="text" placeholder="Full Name" name="username" onChange={handleChange}/>
          </div>
        </div>
        <div className="auth-inputs">
          <div className="auth-input">
        ...
        <button type ="submit" className="auth-submit auth-yellow">Create Account</button>
        </form>
        <Link to="/login" className="auth-switch">Already Signed Up? <span>Login</span></Link>
      </div>
    </div>
  )
  }